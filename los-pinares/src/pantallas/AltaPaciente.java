		...
		if (esCodigoValido(tfCodPaciente.getText().trim())){
			if ((tfNombrePaciente.getText().trim().length() <=30 && tfNombrePaciente.getText().trim().length() >0)){
				...
				if (existePaciente){
					...
					JOptionPane.showMessageDialog(null, "¡El código de paciente ingresado ya existe!","Error", JOptionPane.ERROR_MESSAGE);
				}else{
					//El paciente no existe -> se peude grabar!
					try{
						//Inicia grabación en archivo
						FileWriter datopac = new FileWriter(pathdatosPac,true);
						String registro = Encriptacion.Encriptar(tfCodPaciente.getText().trim() + "," + tfNombrePaciente.getText().trim());								
					...
					JOptionPane.showMessageDialog(null, "¡Alta realizada satisfactoriamente!","Confirmación", JOptionPane.INFORMATION_MESSAGE);
					return true;
				}
			}else{
				//Error: La cantidad de caracteres o el tipo en el nombre son incorrectos -> mostrar
				JOptionPane.showMessageDialog(null, "¡Debe ingresar un nombre de paciente de hasta 30 caracteres y los mismos deben ser letras!","Error", JOptionPane.ERROR_MESSAGE);
			}
		}else{
			//Error: La cantidad o tipo de digitos para el codigo son invalidos -> Mostrar
			JOptionPane.showMessageDialog(null, "¡Debe ingresar un código de paciente numérico de 4 dígitos y mayor a 0!","Error", JOptionPane.ERROR_MESSAGE);
		}		
		return false;
	}
